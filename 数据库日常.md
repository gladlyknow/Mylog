...
4yvxy8q9u5n0m
execute dbms_stats.gather_index_stats(ownname => 'orcl_PM', indname =>'IND_S_DEV_IR_APPNO', estimate_percent =>DBMS_STATS.AUTO_SAMPLE_SIZE,degree=>8);
...
select sum(bytes/1024/1024/1024) from dba_segments where owner='XXX';
...
select  LAST_ANALYZED,table_name from dba_tables where owner='orcl' AND LAST_ANALYZED=SYSDATE-1
...
from dba_indexes where owner='orcl'  and table_name='&1';
and ( LAST_ANALYZED is null or LAST_ANALYZED < to_date('2005-09-11','YYYY-MM-DD') )
set wra on
set linesize 140
set long 99999
...
set linesize 120
set pagesize 9999
SELECT DBMS_METADATA.GET_DDL('SEQUENCE','SEQ_F_YPGT_COMPLETE_STAT ','orcl') FROM DUAL; 
...
FUCK_Y3_IDX                                                                                   98130      98303         99
FUCK_Y4_IDX                                                                                  100180     100351         99
FUCK_Y2_IDX                                                                                  121682     121855         99
FUCK_Y1_IDX                                                                                  145531     145663         99
-----------------------
 单个文件使用率：
col file_name format a50
set linesize 150
...
in('CACHE_FK_DATA','UNDOTBS2','FUCK_Y3_IDX','FUCK_Y4_IDX','FUCK_Y2_IDX','FUCK_Y1_IDX')
 group by file_id) b 
...
alter system set local_listener='(address=(protocol=tcp)(host=db2)(port=1521))' sid='
(address=(protocol=tcp)(host=pmhnrdb1)(port=1521)
alter system set local_listener='(address=(protocol=tcp)(host=test1)(port=1521))' sid='cm1'
alter system set local_listener='(address=(protocol=tcp)(host=192.168.4.105)(port=1521))' sid='cm2'
alter system set local_listener='(address=(protocol=tcp)(host=192.168.3.6)(port=3521))' sid='rdb2';
alter system set remote_listener='(address=(protocol=tcp)(host=192.168.3.8)(port=3521))' sid='pdb1';
alter system set remote_listener='(address=(protocol=tcp)(host=192.168.3.7)(port=3521))' sid='db2';
mv adump adump.bak
mkdir adump
rm -rf adump

alter system set local_listener='(address=(protocol=tcp)(host=db1)(port=1521))' 
 添加删除日志组
...
'(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=193.168.1.10)(PORT=1526))))' sid='UPM1';
 (ADDRESS=(PROTOCOL=TCP)(HOST=193.168.1.10)(PROT=1526))
...
alter system set local_listener ='(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.2.243)(PORT=11521))))' sid='dba1';
alter system set local_listener ='(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.2.245)(PORT=11521))))' sid='dba2';

local_listener                       string      (DESCRIPTION=(ADDRESS=(PROTOCO                                                 L=tcp)(HOST=192.168.2.244)(PORT
                                                 =11521)))
alter system set local_listener ='(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.4.148)(PORT=3521))))' sid='+ASM2';

alter system set local_listener ='(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.4.148)(PORT=3521))))' sid='dba3';
...
order by 1;
 ...
 分区表查询
select PARTITION_NAME,SUBPARTITION_NAME from dba_tab_subpartitions where table_name='' and owner='orcl';

 查询未分析的表

select table_name,num_rows, LAST_ANALYZED,PARTITIONED  from dba_tables where owner='orcl' 
...
alter database create datafile '/oracle/app/oracle/product/11.2.0/db_1/dbs/UNNAMED01089' as '+DBDATA1' size 30G
...
交易平台数据库（192.168.31.41/42/44/45/47）新建表空间 COGNOS_DATA 和 COGNOS_INDEX （均
...
alter system set local_listener='(DESCRIPTION=(ADDRESS_LIST=(ADDRESS=(PROTOCOL=TCP)(HOST=192.168.6.87)(PORT=11521))))' sid='bpmdb_1';
...
select * from dba_constraints where table_name='C_METER' and owner='orcl';
select * from dba_constraints where table_name='C_METER' and owner='orcl';
set linesize 150
set wra off
...
exec dbms_stats.GATHER_TABLE_STATS('orcl','A_RCVBL_ENTRY',estimate_percent=>5,degree=>16);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_A_RCVBL_ENTRY_ACCT_YM',estimate_percent=>20,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_S_BATCH_CUST_APP_ON',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_TEST_CONS_NO',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','PK_S_BATCH_CUST_APP',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_TABLE_STATS('orcl','S_REGION_OUTAGE',estimate_percent=>5,cascade=>true,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_S_APP_APP_NO',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_S_APP_CONS_NO',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_S_APP_CUST_ID',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','IND_S_APP_ON',estimate_percent=>5,degree=>8);
exec dbms_stats.GATHER_INDEX_STATS('orcl','PK_S_APP',estimate_percent=>5,degree=>8);
...
测试调整：
exec dbms_stats.set_table_prefs('orcl','WF_PROCESS','ESTIMATE_PERCENT',20);
exec dbms_stats.set_table_prefs('orcl','WF_ASSIGNMENT_END','ESTIMATE_PERCENT',20);
exec dbms_stats.set_table_prefs('orcl','WF_ACTIVITY','ESTIMATE_PERCENT',20);
select  dbms_stats.get_prefs('ESTIMATE_PERCENT',NULL,'WF_ACTIVITY') FROM DUAL ;

修改统计信息收集比例：
exec dbms_stats.set_table_prefs('orcl','WF_PROCESS','STALE_PERCENT',1);
exec dbms_stats.set_table_prefs('orcl','WF_ASSIGNMENT_END','STALE_PERCENT',1);
exec dbms_stats.set_table_prefs('orcl','WF_ACTIVITY','STALE_PERCENT',1);
select  dbms_stats.get_prefs('STALE_PERCENT','orcl','WF_ACTIVITY') FROM DUAL ;
...
and TABLE_OWNER='orcl' AND LAST_ANALYZED<SYSDATE-6 and table_name<>'A_PC_TRAN' 
...
select * from dba_tab_histograms where table_name='R_ATTR_MAINT' and owner='orcl' and column_name='NEW_VALUE' order by ENDPOINT_NUMBER ;
select * from dba_tab_col_statistics where table_name='R_ATTR_MAINT' and owner='orcl';
select count(*) from sgpms.R_ATTR_MAINT where NEW_VALUE='02';
exec dbms_stats.GATHER_TABLE_STATS('orcl','R_ATTR_MAINT',estimate_percent=>20,cascade=>true,method_opt=>'FOR ALL COLUMNS SIZE AUTO',degree=>8);
...
----------
 ...
 CREATE SEQUENCE orcl.SEQ_CONTRACT_NO_43401
...
select /*+rule*/ * from v$access where sid in(select sid from v$session where username='orcl');
...
select NAME,OWNER,COLUMN_NAME from DBA_SUBPART_KEY_COLUMNS where owner='orcl'
分析步骤：
一、恢复源库192.168.52.50 MDS库中12月4日归档后使用LOGMINER挖掘归档日志信息
...
导入统计信息：
exec dbms_stats.create_stat_table (ownname => 'orcl', stattab => 'ST_T1', tblspace => 'USERS'); 
exec dbms_stats.export_table_stats(ownname =>'orcl',tabname=>'A_RCVBL_ENTRY',stattab=>'ST_T1',statid => 'A');
exec dbms_stats.import_table_stats(ownname => 'orcl', tabname => 'A_RCVBL_ENTRY', stattab => 'ST_T1', statid => 'A', no_invalidate => true); 
...
LISTENER1_PMHSDB01 =
  (DESCRIPTION_LIST =
    (DESCRIPTION =
      ...
      (ADDRESS = (PROTOCOL = TCP)(HOST = 192.168.49.111)(PORT = 11521)(IP = FIRST))
    )
  )
...
SQL> desc x$kffxp
 ...
 FLAGS_KFFXP                                        NUMBER
 CHK_KFFXP                                          NUMBER
 SIZE_KFFXP                                         NUMBERorclorclorclorcl
